...
func NewIndexedFromDb(trkID string, db *gorm.DB) (idx Indexed) {
	db.Raw("select file as f, title, il.name as album, ia.name as artist, iso.length as duration from indexed_songs as iso left join indexed_albums as il on il.id = iso.album left join indexed_artists as ia on ia.id = iso.artist where iso.id = ?", trkID).Scan(&idx)
	return
}
